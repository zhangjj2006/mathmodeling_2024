...
def visualize_results(df, centers_original, silhouette_avg, dbi, ch_index):
    ...
    for i in range(5):
        ...
        axes[1, 0].scatter(cluster_data['BMI'], cluster_data['对应达标天数'], 
    ...
    # 4. 评估指标展示
    metrics = ['轮廓系数', 'DBI指数', 'CH指数']
    values = [silhouette_avg, dbi, ch_index]
...
def main():
    # 请替换为您的Excel文件路径
    file_path = "python_code/bmi_Y_result.xlsx"  # 修改为实际文件路径
    ...
    print("\n聚类摘要:")
    for i in range(5):
        cluster_data = df_result[df_result['Cluster'] == i]['BMI']
        print(f"聚类 {i}: {len(cluster_data)}个样本, BMI范围: {cluster_data.min():.2f}-{cluster_data.max():.2f}, 中心值: {centers_original[i][0]:.2f}")
