...
def calculate_ahp_weights(matrix):
    """
    根据判断矩阵计算AHP权重，并进行一致性检验
    :param matrix: NumPy数组形式的判断矩阵
    :return: 权重向量和一致性比率CR
    """
    n = matrix.shape[0]
    ...
    ri = ri_table.get(n, 1.51) # 如果n>10，取一个近似值
...
def generate_stacked_bmi_mixed_charts_with_ahp_weights():
    # --- 1. 定义判断矩阵并计算权重 ---
    # 基于业务逻辑: always_can > middle > cannot
    #             | always_can | middle | cannot
    # ------------|------------|--------|--------
    # always_can  |      1     |    3   |    5
    # middle      |     1/3    |    1   |    3
    # cannot      |     1/5    |   1/3  |    1
    ...
    # 创建权重字典
    objective_weights_dict = {category: weight for category, weight in zip(category_order, ahp_weights)}
    ...
    for bmi_cat in bmi_categories:
        ...
        if stacked_data:
            ...
            bins_count = min(100, max(50, int(data_range / 3))) if data_range > 0 else 50
            ax1.hist(stacked_data, bins=bins_count, stacked=True, color=stacked_colors, label=stacked_labels, alpha=0.95, edgecolor='black', linewidth=0.1)
        ...
        if len(all_days_raw) > 1:
            ...
            try:
                ...
                ax2.plot(x_range, kde_values, color='dodgerblue', linewidth=2, linestyle='-', label='AHP加权概率密度')
                ...
                for prob in prob_levels:
                    try:
                        ...
                        ax2.axvline(x=day_value, color='purple', linestyle='--', linewidth=1)
                        ax2.text(day_value + 0.5, max(kde_values) * (1-prob), f'{int(prob*100)}% -> {day_value:.1f}天', color='purple', rotation=90)
                    except IndexError:
                        pass
