...
def plot_final_model_comparison(model_results_df):
    ...
    bars1 = ax.bar(x_pos - width/2, model_results_df['auc'], width, label='AUC', color=colors[0], alpha=0.8)
    bars2 = ax.bar(x_pos + width/2, model_results_df['f1'], width, label='F1 Score', color=colors[1], alpha=0.8)
    
    ax.set_xlabel('模型')
    ax.set_ylabel('分数')
    ax.set_title('不同模型性能比较')
    ax.set_xticks(x_pos)
    ax.set_xticklabels(model_results_df['model'])
    ax.legend()
    ax.grid(True, alpha=0.3)
    
    # 在柱子上添加数值
    for bar, auc_val in zip(bars1, model_results_df['auc']):
        height = bar.get_height()
        ax.text(bar.get_x() + bar.get_width()/2., height + 0.01,
               f'{auc_val:.3f}', ha='center', va='bottom', fontsize=9)
    
    for bar, f1_val in zip(bars2, model_results_df['f1']):
        height = bar.get_height()
        ax.text(bar.get_x() + bar.get_width()/2., height + 0.01,
               f'{f1_val:.3f}', ha='center', va='bottom', fontsize=9)
    ...
    plt.tight_layout()
    plt.savefig('model_radar_chart_comparison.png')
    plt.close()
