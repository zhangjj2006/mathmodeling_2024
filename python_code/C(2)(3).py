...
def calculate_percentiles():
    ...
    for bmi_cat in bmi_categories:
        ...
        if df_bmi.empty:
            print(f"警告: BMI区间 {bmi_cat} 没有数据")
            results[bmi_cat] = {'90%': None, '95%': None}
            continue
        ...
        # 计算90%和95%分位数
        if len(all_days) > 0:
            percentile_90 = np.percentile(all_days, 90)
            percentile_95 = np.percentile(all_days, 95)
            results[bmi_cat] = {'90%': percentile_90, '95%': percentile_95}
            
            print(f"BMI区间 {bmi_cat}:")
            print(f"  数据点数量: {len(all_days)}")
            print(f"  90%分位数: {percentile_90:.2f} 天")
            print(f"  95%分位数: {percentile_95:.2f} 天")
            print()
        else:
            results[bmi_cat] = {'90%': None, '95%': None}
            print(f"BMI区间 {bmi_cat}: 没有数据")
